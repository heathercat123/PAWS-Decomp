package entities.enemies
{
   ...
   import sprites.enemies.AnglerFishEnemySprite;
   ...
   public class AnglerFishEnemy extends Enemy
   {
      ...
      public function AnglerFishEnemy(_level:Level, _xPos:Number, _yPos:Number, _direction:int, _ai_index:int)
      {
         ...
         speed = 0.8;
         MAX_Y_VEL = 0.5;
         sprite = new AnglerFishEnemySprite();
      ...
      override public function update() : void
      {
         ...
         if(stateMachine.currentState == "IS_WALKING_STATE")
         {
            ...
            if(Utils.CurrentLevel != 253)
            {
               hero_mid_x = level.hero.xPos + level.hero.WIDTH * 0.5;
               mid_x = xPos + WIDTH * 0.5;
               if(Math.abs(hero_mid_x - mid_x) < 48 && level.hero.yPos <= yPos - Utils.TILE_HEIGHT && level.hero.yPos >= yPos - Utils.TILE_HEIGHT * 4)
               {
         ...
         else if(stateMachine.currentState == "IS_JUMPING_STATE")
         {
            ...
            if(this.ease_tick >= 1.5)
            {
               ...
               level.particlesManager.pushParticle(new SplashParticleSprite(0),xPos + WIDTH * 0.5,Utils.SEA_LEVEL,0,0,0);
               stateMachine.performAction("END_ACTION");
            }
         }
