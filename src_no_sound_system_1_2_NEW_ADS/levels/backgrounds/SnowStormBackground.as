package levels.backgrounds
{
   ...
   public class SnowStormBackground extends Background
   {
      ...
      protected var sky_sin_value:Number;
      
      protected var sky_layer:Image;
      
      public function SnowStormBackground(_level:Level)
      {
         super(_level);
         ...
         this.INIT_SNOW = true;
         this.sky_sin_value = Math.random() * Math.PI * 2;
      ...
      override public function update() : void
      {
         ...
         super.update();
         this.sky_sin_value += 0.02;
         if(this.sky_sin_value > Math.PI * 2)
         {
            this.sky_sin_value -= Math.PI * 2;
         }
         var gradient_value:Number = Math.sin(this.sky_sin_value);
         if(gradient_value > 0.1)
         {
            gradient_value = 0.1;
         }
         else if(gradient_value < -0.3)
         {
            gradient_value = -0.3;
         }
         Starling.current.stage.color = 0;
         ...
         if(this.WIND_GO_FASTER)
         {
            ...
            if(this.wind_extra_force >= 4)
            {
               this.wind_extra_force = 4;
            }
         }
         else
         {
            this.wind_extra_force -= 0.025;
            if(this.wind_extra_force <= 1)
            {
               this.wind_extra_force = 1;
            }
         }
