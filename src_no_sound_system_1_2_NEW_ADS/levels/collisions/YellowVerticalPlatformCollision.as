package levels.collisions
{
   ...
   public class YellowVerticalPlatformCollision extends Collision
   {
      ...
      public function YellowVerticalPlatformCollision(_level:Level, _xPos:Number, _yPos:Number, _dir:int, _isTickTock:int = 0)
      {
         ...
         {
            sprite = new YellowPlatformCollisionSprite(2);
            if(_isTickTock == 1)
            {
               this.IS_TICK = true;
         ...
         {
            sprite = new YellowPlatformCollisionSprite(1);
         }
      ...
      protected function updateTickTock() : void
      {
      }
         ...
         for(i = 0; i < this._entities.length; i++)
         {
            if(this._entities[i] != null)
            {
               if(this._entities[i].entity != null)
               {
                  ...
                  {
                     y_diff = yPos - this.oldYPos;
                     this._entities[i].entity.yPos += y_diff;
                  }
               }
            }
         }
      ...
      override public function checkEntitiesCollision() : void
      {
         ...
         if(entityAABB.intersects(thisAABB) && this._entities[index].entity.allowPlatformCollision(this))
         {
            ...
            {
               this._entities[index].entity.xPos = xPos + aabb.x + aabb.width - 2 - this._entities[index].entity.aabbPhysics.x;
            }
            else
            {
               this._entities[index].entity.xPos = xPos - (this._entities[index].entity.aabbPhysics.x + this._entities[index].entity.aabbPhysics.width);
            }
            this._entities[index].entity.xVel = 0;
            this._entities[index].entity.setOnWallPlatform(this);
            this._entities[index].IS_COLLIDING = true;
         }
         else if(this._entities[index].IS_COLLIDING)
         {
            this._entities[index].entity.IS_ON_VER_PLATFORM = false;
         ...
         for(i = 0; i < level.enemiesManager.enemies.length; i++)
         {
            ...
            {
               entityAABB = new Rectangle(level.enemiesManager.enemies[i].xPos + level.enemiesManager.enemies[i].aabbPhysics.x,level.enemiesManager.enemies[i].yPos + level.enemiesManager.enemies[i].aabbPhysics.y,level.enemiesManager.enemies[i].aabbPhysics.width,level.enemiesManager.enemies[i].aabbPhysics.height);
               if(entityAABB.intersects(thisAABB) && Boolean(level.enemiesManager.enemies[i].allowPlatformCollision(this)))
               {
                  level.enemiesManager.enemies[i].wallCollision();
               }
            }
         }
      }
